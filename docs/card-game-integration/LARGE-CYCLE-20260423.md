...
Run one real autopilot -> relay cycle against `D:\Unity\card game` without tailing full logs, then record the failures that still block safe operator use.

## What was run

- Unity preflight:
  - `.autopilot/project.ps1 doctor`
  - live Unity MCP checks: telemetry + console
...
- The relay session `companion-depth-first-slice-20260423-112716` stayed `Active` on turn 2.
...
- The waiter eventually returned:
...
2. Prepared-session state and active-relay state could diverge.
   ...
   - That makes the operator surface harder to trust.
...
3. Unity MCP usage was not being recorded as compact evidence.
   ...
   - `Write-CardGameAutopilotResult.ps1` also hard-coded `mcp_calls = 0`.

...
- For `qa-editor` or `Tools/QA/*` tasks, perform at least one Unity MCP verification action before handoff when MCP is available. Preferred first action: `read_console` or `execute_menu_item` on the narrow QA menu you are validating.
